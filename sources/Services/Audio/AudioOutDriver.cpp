...
void AudioOutDriver::clipToMix() {
    ...
    if (!hasSound_) {
        ...
        fixed peakFixed = 0;
        int limiterCount = 0;
        const float SAFETY_THRESHOLD = 0.99f; // normalized (0..1)
        fixed safetyThresholdFixed = (fixed)((float)MAX_POSITIVE_FIXED * SAFETY_THRESHOLD);
        ...
        for (int i = 0; i < sampleCount_; i++) {
            ...
            // Safety limiter: per-sample scale-down when exceeding threshold
            {
                fixed al = l < 0 ? -l : l;
                fixed ar = r < 0 ? -r : r;
                if (al > safetyThresholdFixed) {
                    float scale = (float)safetyThresholdFixed / (float)al;
                    l = (fixed)((float)l * scale);
                    limiterCount++;
                }
                if (ar > safetyThresholdFixed) {
                    float scale = (float)safetyThresholdFixed / (float)ar;
                    r = (fixed)((float)r * scale);
                    limiterCount++;
                }
            }
        ...
        if (limiterCount > 0) {
            /* safety limiter applied — debug message removed */
        }
