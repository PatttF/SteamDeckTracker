...
void ThemeView::DrawView() {
    ...
    // This runs before the modal's Redraw(), so modal content will draw on top correctly.
    {
        ...
        if (imp) {
            ...
            imp->SetColor(bg);
            // DrawRect expects GUI coordinates (not pixels). Use GUI units
            // so the internal transform handles scaling and app anchor.
            for (int i = 0; i < THEME_COLOR_COUNT; i++) {
                int gx = anchor._x + 11;
                int gy = anchor._y + 1 + i;
                GUIRect r(gx, gy, gx + 2, gy + 1);
                imp->DrawRect(r);
            }
        }
    }
