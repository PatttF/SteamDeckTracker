...
#include "AscentPresetData.h"
#include "DolosPresetData.h"
...
void VST3Effect::SyncHostVolume(int hostVol) {
    // hostVol: 0..255
    double norm = (double)hostVol / 255.0;
    if (norm < 0.0) norm = 0.0;
    if (norm > 1.0) norm = 1.0;

    // Update wrapper volume variable for UI consistency
    Variable *volVar = FindVariable(VST3FX_VOLUME);
    if (volVar) volVar->SetInt(hostVol);

    // Heuristic: find native parameters whose name indicates output/gain/volume
...
void VST3Effect::loadPluginInner() {
    ...
    // IPC (yabridge/Wine) doesn't block the audio render thread.
    {
        ...
        // Force-load preset 0 so the effect actually applies its init
        // preset.  Many plugins don't load any sound automatically after
        // initialize()+setActive().
        if (GetPresetCount() > 0) {
            SetPreset(0);
        }
...
void VST3Effect::discoverHardcodedPresets() {
    ...
    // --- Ascent (Audio Damage) embedded presets ---
    if (strstr(name_, "Ascent") != nullptr) {
        loadValhallaEmbedded(ascentBanks, (int)(sizeof(ascentBanks)/sizeof(ascentBanks[0])), programLists_, embeddedXmlByBank_);
        usingEmbeddedPresets_ = true;
        currentBank_ = 0;
        currentPreset_ = 0;
        return;
    ...
    // --- Dolos (ReplicatAudio) embedded presets ---
    if (strstr(name_, "Dolos") != nullptr) {
        loadValhallaEmbedded(dolosBanks, (int)(sizeof(dolosBanks)/sizeof(dolosBanks[0])), programLists_, embeddedXmlByBank_);
        usingEmbeddedPresets_ = true;
        currentBank_ = 0;
        currentPreset_ = 0;
        return;
    ...
    // (Removed embedded Arturia Efx FRAGMENTS hardcoded presets)
    // The effect will now fall through to the generic file-based preset discovery
    // implemented elsewhere in this module.
}
        ...
        if (currentBank_ < (int)filePresetsByBank_.size() &&
            ...
            for (int m = 0; knownFxPresetMappings[m].pluginNameSubstring != nullptr; m++) {
                if (strstr(name_, knownFxPresetMappings[m].pluginNameSubstring) != nullptr) {
                    ...
                    if (knownFxPresetMappings[m].extension2) {
                        size_t ext2Len = strlen(knownFxPresetMappings[m].extension2);
                        if (fp.filePath.size() > ext2Len &&
                            fp.filePath.substr(fp.filePath.size() - ext2Len) ==
                                knownFxPresetMappings[m].extension2) {
                            headerSkip = knownFxPresetMappings[m].headerSkipBytes2;
                        }
                    }
                    break;
                }
            ...
            if (loadPresetFromFile(fp.filePath, headerSkip)) {
                ...
                // Log full parameter list (name, normalized value, display string)
                if (editController_) {
                    IEditController* ctrl = (IEditController*)editController_;
                    for (int pi = 0; pi < (int)parameters_.size(); ++pi) {
                        double norm = parameters_[pi].currentValue;
                        // Try to get a human-readable string for the value
                        String128 s128;
                        if (ctrl->getParamStringByValue(parameters_[pi].paramId, norm, s128) == kResultOk) {
                            char buf[128];
                            str128ToChar(s128, buf, 128);
                            Trace::Log("VST3", "param[%d] '%s' = %.6f (%s)",
                                       pi, parameters_[pi].name.c_str(), norm, buf);
                        } else {
                            Trace::Log("VST3", "param[%d] '%s' = %.6f",
                                       pi, parameters_[pi].name.c_str(), norm);
                        }
                    }
                }
            }
...
void VST3Effect::RestoreContent(TiXmlElement *element) {
    ...
    // After plugin is instantiated we may want to sync the wrapper 'volume'
    // into any matching native plugin parameter (if present).  Queue a
    // pending sync so it occurs after discoverParameters() runs.
    Variable *volVar = FindVariable(VST3FX_VOLUME);
    if (volVar) {
        int volVal = volVar->GetInt();
        // Use pendingParamValues_ so discoverParameters/RestoreContent will
        // apply them when parameters exist.
        StorePendingVariable("__HOSTVOL_SYNC__", std::to_string(volVal).c_str());
    }
