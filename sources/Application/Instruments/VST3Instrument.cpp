...
void VST3Instrument::ProcessCommand(int channel, FourCC cc, ushort value) {
    ...
    // ---- MIDI commands routed through QueueMidiEvent ----
        ...
        // MIDI CC7 (Volume) — high byte = speed (ignored here), low byte = target
...
void VST3Instrument::QueueMidiEvent(unsigned char status, unsigned char data1, unsigned char data2) {
    ...
    // Handle note-on and note-off as PendingNoteEvents for polyphonic MIDI input
    if (type == 0x90 && data2 > 0) {
        PendingNoteEvent evt;
        evt.channel = 0;
        evt.pitch = data1 & 0x7F;
        evt.velocity = (data2 & 0x7F) / 127.0f;
        evt.noteId = evt.pitch;
        pendingEventsMutex_.Lock();
        pendingNoteEvents_.push_back(evt);
        pendingEventsMutex_.Unlock();
        return;
    ...
    switch (type) {
        ...
        case 0xC0: // Program change — maps to kCtrlProgramChange (130)
            controllerNumber = 130;
            normalizedValue = (data1 & 0x7F) / 127.0;
            break;
        default:
            return;
...
void VST3Instrument::loadPluginInner() {
    ...
    // the semaphore.
    {
        ...
        Trace::Log("VST3", "post-discoverPresets: plugin='%s' presetCount=%d usingFilePresets=%d usingEmbeddedPresets=%d usingCompressedPresets=%d usingMidiPresets=%d",
                   name_, presetCount, usingFilePresets_ ? 1 : 0, usingEmbeddedPresets_ ? 1 : 0, usingCompressedPresets_ ? 1 : 0, usingMidiPresets_ ? 1 : 0);
        if (presetCount > 0) {
            Trace::Log("VST3", "forcing SetPreset(0) for plugin '%s' (presetCount=%d)", name_, presetCount);
            SetPreset(0);
        }
    }
}
...
void VST3Instrument::discoverHardcodedPresets() {
    // Diagnostic: log entry for TAL family so we can confirm this routine runs
    if ((name_ && strstr(name_, "TAL") != nullptr) || (pluginPath_ && strstr(pluginPath_, "TAL") != nullptr)) {
    ...
    // --- TAL U-No family: embedded XML + setState (TAL-U-NO, U-No-LX, etc.) ---
    {
        const char *n = name_;
        const char *pp = pluginPath_;
        bool isUno = false;
        // Accept multiple name/path variants (explicitly include U-No-LX / V2 variants)
        if ((n && (strstr(n, "U-No") != nullptr || strstr(n, "U-NO") != nullptr || strstr(n, "UNo") != nullptr || strstr(n, "U-No-LX") != nullptr || strstr(n, "U-No-LX-V2") != nullptr || strstr(n, "U-NO-LX") != nullptr || strstr(n, "U-NO-LX-V2") != nullptr || strstr(n, "TAL U-NO") != nullptr || strstr(n, "TAL-U-NO") != nullptr || strstr(n, "TAL U-No-LX") != nullptr || strstr(n, "TAL U-No-LX-V2") != nullptr)) ||
            (pp && (strstr(pp, "TAL-U-NO") != nullptr || strstr(pp, "tal-u-no") != nullptr || strstr(pp, "TAL_U_NO") != nullptr || strstr(pp, "U-No-LX") != nullptr || strstr(pp, "U-No-LX-V2") != nullptr))) {
            isUno = true;
        }

        if (isUno) {
            // Group TAL-U-NO embedded presets into banks using the XML's
            // category/path attributes (e.g. "TAL Presets Bank/Pad").
            programLists_.clear();
            embeddedXmlByBank_.clear();

            std::vector<std::string> bankOrder;
            std::map<std::string,int> bankIndex;
            std::vector<std::vector<const char*>> xmlByBank;
            std::vector<VST3ProgramList> lists;

            for (int pidx = 0; pidx < TAL_UNO_EMBEDDED_COUNT; pidx++) {
                const char *xml = talUnoEmbeddedPresets[pidx].xmlData;
                const char *pname = talUnoEmbeddedPresets[pidx].name;

                // Prefer category="..." (inside <program>) then fall back to
                // the <tal path="..."> attribute.  Extract the last path
                // component (e.g. "Pad", "Lead", "Bass").
                const char *catStart = strstr(xml, "category=\"");
                std::string bankName;
                if (catStart) {
                    catStart += strlen("category=\"");
                    const char *catEnd = strchr(catStart, '"');
                    if (catEnd && catEnd > catStart) {
                        std::string cat(catStart, catEnd - catStart);
                        size_t pos = cat.find_last_of('/');
                        if (pos != std::string::npos && pos + 1 < cat.size())
                            bankName = cat.substr(pos + 1);
                        else
                            bankName = cat;
                    }
                } else {
                    const char *pathStart = strstr(xml, "path=\"");
                    if (pathStart) {
                        ...
                        if (pathEnd && pathEnd > pathStart) {
                            ...
                            if (lastSlash != std::string::npos) {
                                ...
                                size_t secLast = dir.find_last_of('/');
                                if (secLast != std::string::npos && secLast + 1 < dir.size())
                                    bankName = dir.substr(secLast + 1);
                                else
                                    bankName = dir;
                            } else {
                                bankName = path;
                ...
                if (bankName.empty()) bankName = "Factory Presets";

                int idx;
                auto it = bankIndex.find(bankName);
                if (it == bankIndex.end()) {
                    idx = (int)bankOrder.size();
                    bankIndex[bankName] = idx;
                    bankOrder.push_back(bankName);
                    xmlByBank.emplace_back();
                    lists.emplace_back();
                    lists.back().listId = idx;
                    lists.back().name = bankName;
                } else {
                    idx = it->second;
                }

                lists[idx].programs.push_back(pname);
                xmlByBank[idx].push_back(xml);
            }

            programLists_ = lists;
            embeddedXmlByBank_ = xmlByBank;
            usingEmbeddedPresets_ = true;
            currentBank_ = 0;
            currentPreset_ = 0;
            return;
    ...
    if (strstr(name_, "Odin") != nullptr) {
        programLists_.clear();
        compressedPresets_.clear();
#ifdef ODIN2_BANK_COUNT
        for (int b = 0; b < ODIN2_BANK_COUNT; ++b) {
            VST3ProgramList pl;
            pl.listId = (int)programLists_.size();
            pl.name = odin2Banks[b].name;
            for (int p = 0; p < odin2Banks[b].count; ++p) {
                pl.programs.push_back(odin2Banks[b].presets[p].name);
                CompressedPreset cp;
                cp.data = odin2Banks[b].presets[p].compressedData;
                cp.compressedSize = odin2Banks[b].presets[p].compressedSize;
                cp.originalSize = odin2Banks[b].presets[p].originalSize;
...
void VST3Instrument::SetPreset(int presetIdx) {
    ...
    Trace::Log("VST3", "SetPreset called for plugin '%s' bank=%d preset=%d",
               name_, currentBank_, currentPreset_);

    // --- Embedded XML preset loading (TAL-NoiseMaker, Valhalla, etc.) ---
    if (usingEmbeddedPresets_) {
        // Prefer banked XML (embeddedXmlByBank_) when available (Valhalla/TAL)
        if (currentBank_ >= 0 && currentBank_ < (int)embeddedXmlByBank_.size()) {
            if (presetIdx >= 0 && presetIdx < (int)embeddedXmlByBank_[currentBank_].size()) {
                const char *xml = embeddedXmlByBank_[currentBank_][presetIdx];
                if (xml) loadPresetFromXmlData(xml);
                return;
            }
        }
        // Fallback to single-array embeddedXmlPresets_
        if (presetIdx >= 0 && presetIdx < (int)embeddedXmlPresets_.size() &&
    ...
    if (usingFilePresets_) {
        Trace::Log("VST3", "using file preset for '%s' -> filePresetsByBank size=%zu",
               name_, filePresetsByBank_.size());
        if (currentBank_ < (int)filePresetsByBank_.size() &&
            ...
            Trace::Log("VST3", "loading preset file '%s' (name='%s') for plugin '%s'",
                       fp.filePath.c_str(), fp.name.c_str(), name_);
            ...
            if (loadPresetFromFile(fp.filePath, headerSkip)) {
                Trace::Log("VST3", "loadPresetFromFile returned true for '%s'", fp.filePath.c_str());
            }
        }
        return;
    }
    ...
    if (usingMidiPresets_) {
        Trace::Log("VST3", "using MIDI presets for plugin '%s' bank=%d preset=%d",
               name_, currentBank_, presetIdx);
    ...
    for (int32 i = 0; i < paramCount; i++) {
        ...
        Trace::Log("VST3", "program-change via param id=%u normalized=%f for plugin '%s'",
               (unsigned)info.id, normalized, name_);
