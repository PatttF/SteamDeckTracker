...
InstrumentBank::InstrumentBank():Persistent("INSTRUMENTBANK") {
	...
	for (int i=0;i<MAX_MIDIOUTINSTRUMENT_COUNT;i++) {
    ...
    // initialise per-instrument MIDI-input stash (no binding by default)
    for (int si = 0; si < MAX_INSTRUMENT_COUNT; ++si) {
        savedIMDI_[si] = -1;
        savedIMIC_[si] = -1;
    }
}
...
void InstrumentBank::SetInstrumentType(int i, InstrumentType type) {
    ...
    // Stash current IMDI/IMIC so temporary switches (e.g. ->AudioIn) don't lose the binding.
    Variable *stashDev = old->FindVariable(MAKE_FOURCC('I','M','D','I'));
    savedIMDI_[i] = stashDev ? stashDev->GetInt() : -1;
    Variable *stashCh = old->FindVariable(MAKE_FOURCC('I','M','I','C'));
    savedIMIC_[i] = stashCh ? stashCh->GetInt() : -1;
    ...
    // Restore IMDI/IMIC from the per-instrument stash (savedIMDI_/savedIMIC_)
    // or from any variable that was copied above. Only insert/apply these
    // when the *new* instrument type actually accepts MIDI input; otherwise
    // keep values in the stash for future restore.
    if (n->GetType() != IT_MIDI && n->GetType() != IT_MIDIOUT && n->GetType() != IT_AUDIOIN) {
        int restoreDev = savedIMDI_[i];
        int restoreCh  = savedIMIC_[i];

        // If iterator copy already copied IMDI/IMIC into the new instrument,
        // prefer those values and update the stash so it stays consistent.
        Variable *newDev = n->FindVariable(MAKE_FOURCC('I','M','D','I'));
        if (newDev) {
            savedIMDI_[i] = newDev->GetInt();
        } else if (restoreDev >= 0) {
            newDev = new Variable("midi dev", MAKE_FOURCC('I','M','D','I'), restoreDev);
            n->Insert(newDev);
            /* Trace::Debug removed */
        }

        Variable *newCh = n->FindVariable(MAKE_FOURCC('I','M','I','C'));
        if (newCh) {
            savedIMIC_[i] = newCh->GetInt();
        } else if (restoreCh >= 0) {
            newCh = new Variable("midi ch", MAKE_FOURCC('I','M','I','C'), restoreCh);
            n->Insert(newCh);
            /* Trace::Debug removed */
        }
    } else {
    ...
    // --- Ensure MIDI router clears/rebuilds any stale state for this
    // instrument index after the type-change. This prevents cases where
    // plugin/live slots still reference the old instrument type and
    // incoming MIDI gets hung up when switching back and forth.
    GUIWindow *gw = Application::GetInstance()->GetWindow();
    AppWindow *aw = nullptr;
    if (gw) aw = dynamic_cast<AppWindow *>(gw);
    if (aw && aw->GetMidiInputRouter()) {
        aw->GetMidiInputRouter()->ClearInstrumentRouting(i);
        /* Trace::Debug removed */
    }
}
